...
abstract class Log {
    ...
    class Context {
        ...
        val variables = mutableListOf<Pair<String, JsonElement>>()
        ...
        fun data(name: String, value: Boolean?) {
            variables.add(name to JsonPrimitive(value))
        }

        fun data(name: String, value: Int?) {
            variables.add(name to JsonPrimitive(value))
        }

        fun data(name: String, value: Long?) {
            variables.add(name to JsonPrimitive(value))
        }

        fun data(name: String, value: Double?) {
            variables.add(name to JsonPrimitive(value))
        }

        fun data(name: String, value: UUID?) {
            variables.add(name to JsonPrimitive(value?.toString()))
        }
    ...
    abstract fun child(name: String): Log

    class WithContext(val base: Log, val parentCtx: Context) : Log() {
        override fun child(name: String): Log = WithContext(base.child(name), parentCtx)
    ...
    fun withContext(build: Context.() -> Unit): WithContext =
        WithContext(this, Context().apply(build))

    companion object {
        val dummy: Log =
            object : Log() {
                override fun child(name: String): Log = this
...
sealed class Set1<out T> {
    ...
    data class Prim<out T>(val on: Set<T>) : Set1<T>()

    operator fun contains(v1: @UnsafeVariance T): Boolean =
        when (this) {
            is All -> true
...
class LogManager(
    ...
    var bottomFrameBlacklist: List<String> = listOf("java.lang.Thread"),
) {
    private fun cleanupStackTraceDynamic(stack: Array<StackTraceElement>): List<StackTraceElement> {
        val queue = ArrayDeque<StackTraceElement>().apply { stack.forEach { add(it) } }
        while (queue.isNotEmpty()) {
            val head = queue.first()
            if (
                head.className == "${this::class.qualifiedName}\$LogImpl" ||
                    head.className == Log::class.qualifiedName
            ) {
                queue.removeFirst()
            } else {
                break
        ...
        while (queue.isNotEmpty()) {
            val tail = queue.last()
            if (bottomFrameBlacklist.any { tail.className.startsWith(it) }) {
                queue.removeLast()
            } else {
                break
            }
    ...
    private fun stackTraceInfo(stack: Array<StackTraceElement>): Log.Stacktrace {
        val adjustedStack =
            stack.dropWhile {
                it.className == "minilog.LogManager\$LogImpl" || it.className == "minilog.Log"
            }
    ...
    private inner class LogImpl(
        ...
        val logger: Logger,
        @Volatile var enabled: Set1<Tag>,
    ) : Log() {
        // FIXME: enabled inheritance?
        override fun child(name: String): Log =
            LogImpl(
                this.name + name,
                Logger.getLogger(this.name.joinToString(".") + "." + name),
                enabled,
            )

        private fun log(ulid: String?, tags: List<Tag>, f: Context.() -> Unit) {
            // Tag gate first
            if (!tags.any { it in enabled }) return
            // JUL gate next
            val effectiveLevel =
                tags
                    .maxOf { it.level.intValue() }
                    .let { n -> tags.first { it.level.intValue() == n }.level }
            if (!logger.isLoggable(effectiveLevel)) return
            ...
            for ((n, v) in ctx.variables) {
                ...
                if (v is CharSequence) {
                    sb.append('"')
                        .append(escapeJavaString(v, doubleQuoted = true, limit = 256))
                        .append('"')
                } else {
                    sb.append(v)
                }
            ...
            fun go(
                ...
                val childStack = cleanupStackTraceDynamic(throwable.stackTrace)
                val parentStack =
                    parent?.let { cleanupStackTraceDynamic(it.stackTrace) } ?: emptyList()

                if (parent == null) {
                    for (s in childStack) {
                        sb.append("  ".repeat(depth + 1)).append(s.toString()).append(NL)
                    }
                } else {
                    val common =
                        parentStack
                            .asReversed()
                            .zip(childStack.asReversed())
                            .takeWhile { (a, b) -> a == b }
                            .size
            ...
            // Logging.
            // //////////////////////////////////////////////////////////////////
            logger.logp(effectiveLevel, stack.className, stack.methodName, message)
        }
    ...
    fun getLogger(name: String): Log =
        loggers.computeIfAbsent(name) { LogImpl(listOf(name), Logger.getLogger(name), Set1.All) }

    fun setEnabled(name: String, enabled: Set1<Log.Tag>) {
        loggers
            .computeIfAbsent(name) { LogImpl(listOf(name), Logger.getLogger(name), Set1.All) }
            .enabled = enabled
    }
}
